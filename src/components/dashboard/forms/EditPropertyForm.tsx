...
const EditPropertyForm = () => {
    ...
    const handleDeletePotential= (x:IPotentialProperty)=>{
        dashboard.deletePotentialProperty(x);
    }

    const handleDelete= async (id:number)=>{
        await dashboard.deleteProperty(id);
    }
    ...
    return (
        <>
            <Space direction={'vertical'} style={{width:'100%', marginTop:20}}>
                ...
                {dashboard.properties.map(x=>
                    <>
                        <Row justify="space-around">
                            <Col span={4}>{x.key?.keyValue}</Col>
                            <Col span={4}>{x.value}</Col>
                            <Col span={4}>
                                <Button onClick={()=>handleDelete(Number(x.id))}>Удалить</Button>
                            </Col>
                        </Row>
                        <Divider/>
                    </>
                ...
                {dashboard.potentialProperties.map(x=>
                    <>
                        <Row justify="space-around">
                            ...
                            <Col span={4}>{x.value}</Col>
                            <Col span={4}>
                                <Button onClick={()=>handleDeletePotential(x)}>Удалить</Button>
                            </Col>
                        </Row>
                        <Divider/>
                    </>
...
export default observer(EditPropertyForm);